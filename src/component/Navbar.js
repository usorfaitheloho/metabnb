  ...
  return (
        <nav className="bg-white border-gray-200 px-2 pt-4 md:px-4 sm:px-4 py-2.5 rounded dark:bg-gray-900">
            ...
            <span class="flex items-center">
                ...
                <div className={`${menuOpen ? "visible bottom-0 flex flex-col lg:flex-row pt-16 gap-y-12 items-center lg:gap-y-0 fixed lg:static w-[100%] h-full lg:w-max right-0 lg:h-max top-16 bottom-0 bg-[#A02279] text-[#FFFF] z-8 transition-all duration-1000 ease-in-out lg:transition-none" : "hidden justify-between -right-full" } w-[40%] lg:justify-between md:flex text-[#434343] text-sm`}>
                    {data && data.map((item) => (
                        <ul key={item.id}>
